...
from utility import get_id_date_hash
...
class UserMatchesByServerRepository:
    ...
    def add_today_match(self, server_id, user_id, matched_user_id):
        server_date_hash = get_id_date_hash(server_id)
        user_id = str(user_id)
    ...
    def get_matches_at_date(self, server_id, date):
        server_date_hash = get_id_date_hash(server_id, date)
        records = UserMatchesByServer.objects(server_date_hash=server_date_hash).all()
        return {record.user_id: record.user_match_id for record in records} if records else None
    ...
    def get_match_at_date(self, server_id, user_id, date):
        server_date_hash = get_id_date_hash(server_id, date)
        user_id = str(user_id)
        records = UserMatchesByServer.objects(server_date_hash=server_date_hash).all()
        for record in records:
